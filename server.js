...
//         // Find the last array key and its value
//         let lastArrayKey = null;
//         let lastArrayData = null;

//         const findLastArrayKey = (obj) => {
//           for (let key in obj) {
//             if (obj.hasOwnProperty(key)) {
//               if (Array.isArray(obj[key])) {
//                 lastArrayKey = key;
//                 lastArrayData = obj[key];
//               } else if (typeof obj[key] === "object") {
//                 findLastArrayKey(obj[key]);
//               }
//             }
//           }
//         };

//         findLastArrayKey(jsonData);

//         if (!lastArrayKey || !lastArrayData) {
//           return res.status(400).send("No array found in the JSON file");
//         }
...
//         // Flatten and prepare data for CSV writing
//         const flatData = lastArrayData.map((item) => flattenObject(item));
  ...
  if (req.file) {
    ...
    fs.readFile(req.file.path, "utf8", (err, data) => {
      ...
      try {
        ...
        // Function to find the first array within an object in the JSON
        ...
        // Function to find the first array in the JSON data
        const findFirstArrayInJSON = (obj) => {
          if (Array.isArray(obj)) {
            return obj;
          } else if (typeof obj === "object") {
            for (let key in obj) {
              if (obj.hasOwnProperty(key)) {
                const result = findFirstArrayInJSON(obj[key]);
                if (result) {
                  return result;
                }
              }
        ...
        // Find the first array within an object in the JSON
        let arrayData = findArrayInObject(jsonData);

        // If no array is found within an object, find the first array in the JSON data
        if (!arrayData) {
          arrayData = findFirstArrayInJSON(jsonData);
        }
